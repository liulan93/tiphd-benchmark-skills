...
def make_inputs(sc_adata, bulk_adata, layer_name='X'):
    ...
    if layer_name == 'X':
        ...
        if bulk_adata is not None:
            s = torch.tensor(np.array(bulk_adata.X.todense() if hasattr(bulk_adata.X, 'todense') else bulk_adata.X).astype(np.float32))
        else:
            s = None
    else:
        x = torch.tensor(safe_toarray(sc_adata.layers[layer_name]))
        if bulk_adata is not None:
            s = torch.tensor(np.array(bulk_adata.layers[layer_name]).astype(np.float32))
        else:
            s = None
    return x, s
